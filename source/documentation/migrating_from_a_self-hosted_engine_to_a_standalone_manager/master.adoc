...
// [id='Migrating_from_a_Self-hosted_Engine_to_a_Standalone_Manager']
:rhv-doc:
include::common/collateral_files/_attributes.adoc[]
:_content-type: ASSEMBLY
[id="migrating-from-a-self-hosted-engine-to-a-standalone-manager"]
...
. xref:Installing_the_self-hosted_engine_deployment_host_migrating_to_SAM[Install a new host to deploy the self-hosted engine on.] You can use either host type:
...
. xref:Preparing_Storage_for_RHV_migrating_to_SAM[Prepare storage for the self-hosted engine storage domain.] You can use one of the following storage types:
* xref:Preparing_NFS_Storage_migrating_to_SAM[NFS]
* xref:Preparing_iSCSI_Storage_migrating_to_SAM[iSCSI]
* xref:Preparing_FCP_Storage_migrating_to_SAM[Fibre Channel (FCP)]
* xref:Preparing_Red_Hat_Gluster_Storage_migrating_to_SAM[{gluster-storage-fullname}]
...
. xref:Enabling_the_Red_Hat_Virtualization_Manager_Repositories_migrating_to_SAM[Enable the {engine-name} repositories on the new {engine-name} virtual machine.]

. xref:Reinstalling_an_Existing_Host_as_a_Self-Hosted_Engine_Node_migrating_to_SAM[Convert regular hosts to self-hosted engine nodes that can host the new {engine-name}.]
